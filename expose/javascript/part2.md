1. It prints the value of 'i' when exiting the 'for loop' which is 3. Since, it is a variable of type 'var', it is declared at function level and doesn't get deleted after the block is executed
2. It prints the last value taken by 'discountedPrice' when exiting the 'for loop' which is 150. Since, it is a variable of type 'var', it is declared at function level and doesn't get deleted after the block is executed
3. It prints the last value taken by 'finalPrice' when exiting the 'for loop' which is 150. Since, it is a variable of type 'var', it is declared at function level and doesn't get deleted after the block is executed (all 'var' are hoisted so it doesn't matter where it is declared).
4. It will return an array of size 3 with all discounted prices inside ([50, 100, 150]). This is because each iteration we will push the final price (rounded discounted price) inside the array for each iteration.
...
6. An error appears: 'ReferenceError: discountedPrice is not defined'. This happens because 'let' variables are block level and 'discountedPrice' gets deleted after exiting the loop.
7. It prints the last value taken by 'finalPrice' when exiting the 'for loop' which is 150. Since, it is declared at the start of the function its block-level scope is the function scope itself so that's why the varoable doesn't get deleted after exiting the function.
8. It will return an array of size 3 with all discounted prices inside ([50, 100, 150]). This is because each iteration we will push the final price (rounded discounted price) inside the array for each iteration. And since it is declared at the start, the scope of the variable is the function itself.
